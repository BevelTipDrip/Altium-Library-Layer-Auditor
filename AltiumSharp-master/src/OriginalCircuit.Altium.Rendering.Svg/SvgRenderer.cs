...
public sealed class SvgRenderer : IRenderer
{
    ...
    /// <summary>
    /// Renders a single PCB component (footprint), same as <see cref="RenderAsync(IComponent, Stream, RenderOptions?, CancellationToken)"/>,
    /// except each PCB layer's primitives are wrapped in a named SVG group (<c>&lt;g id="layer-33"&gt;</c>).
    /// A front-end can then toggle individual layers on/off client-side, with no server round-trip — the
    /// same trick <see cref="RenderRealisticAsync(PcbDocument, Stream, RenderOptions?, PcbRealisticStyle?, CancellationToken)"/>
    /// uses for its physical (substrate/copper/soldermask/...) groups.
    /// </summary>
    /// <param name="component">The PCB component (footprint) to render.</param>
    /// <param name="output">The stream the SVG is written to.</param>
    /// <param name="options">Width/height/background; null uses the defaults.</param>
    /// <param name="viewSide">Which side's layers to include; <see cref="PcbViewSide.Both"/> (the default)
    /// shows every layer the footprint uses, which is usually what you want for a library footprint.</param>
    /// <returns>The distinct PCB layer IDs that were drawn, in the order their groups appear in the SVG —
    /// pass each through <see cref="LayerColors.GetName"/> / <see cref="LayerColors.GetColor"/> to build a
    /// layer-visibility UI.</returns>
    public async ValueTask<IReadOnlyList<int>> RenderGroupedByLayerAsync(
        PcbComponent component,
        Stream output,
        RenderOptions? options = null,
        PcbViewSide viewSide = PcbViewSide.Both,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(component);
        ArgumentNullException.ThrowIfNull(output);
        ...
        options ??= new RenderOptions();
        IReadOnlyList<int> layers = Array.Empty<int>();
        var bytes = RenderToBytes(options, component.Bounds, 0.9, (transform, ctx) =>
        {
            var renderer = new PcbComponentRenderer(transform) { ViewSide = viewSide };
            layers = renderer.RenderGroupedByLayer(component, ctx);
        });
        await output.WriteAsync(bytes, cancellationToken);
        return layers;
    }
